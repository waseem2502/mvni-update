...
StatusType Ocean::treason(int sourceShipId, int destShipId)
{
    if((sourceShipId <= 0) || (destShipId <= 0) || (destShipId == sourceShipId))
        return StatusType::INVALID_INPUT;
    try {
        node<int, Ship> *source = m_ships.find(sourceShipId);
        node<int, Ship> *target = m_ships.find(destShipId);
...
StatusType Ocean::update_pirate_treasure(int pirateId, int change)
{
    ...
    Ship* currentShip =  current->m_data->getSourceShip();
    node<int,AVLTree<int,shared_ptr<Pirates>>>* treasure_Tree = currentShip->pirates_treasure_tree.find(current->m_data->getTreasure());

    treasure_Tree->m_data.remove(pirateId);
    if(treasure_Tree->m_data.getSize() == 0)
        treasure_Tree->m_data.remove(change);
    change -= current->m_data->getSourceShip()->getCoinDiff();
...
output_t<int> Ocean::get_richest_pirate(int shipId)
{
    ...
    return output_t<int>(m_ships.find(shipId)->m_data.pirates_treasure_tree.searchMax()
    ->m_data.searchMax()->m_key);
}
...
StatusType Ocean::ships_battle(int shipId1,int shipId2)
{
    ...
    node<int,Ship>* ship1 = m_ships.find(shipId1);
    node<int,Ship>* ship2 = m_ships.find(shipId2);
